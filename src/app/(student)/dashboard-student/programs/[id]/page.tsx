...
export default function StudentProgramDetailPage() {
  ...
  // Fetch all student courses and filter by programId client-side
  const { data: coursesData, isLoading: coursesLoading } = useQuery({
    queryKey: ["student-courses-all-paginated", 1, 100],
    queryFn: () => fetchCourses(1, 100),
  });
  ...
  const courses = useMemo(
    () => (coursesData?.data ?? []).filter((c) => {
      ...
      // The course type doesn't expose programId directly, but the program name can match
      // We rely on the enrollment's program courses list as the source of truth
      const programCourseNames = enrollment?.program?.courses?.map(
        (pc) => pc.courseTitle ?? pc.name ?? ""
      ) ?? [];
      return programCourseNames.includes(c.name);
    }),
    [coursesData, enrollment]
  );

  if (enrollmentsLoading) return <Loading withOverlay={false} description="Loading program..." />;
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <h1 className={styles.title}>{enrollment.program?.name ?? "Program"}</h1>
        <div className={styles.meta}>
          <span>Head: {enrollment.program?.headOfProgram?.fullName ?? "—"}</span>
          <span>·</span>
